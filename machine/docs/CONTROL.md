...
typedef struct {
    ...
    int rotate_num;               // Rotation degrees (time-based)
} SessionConfig_t;
```
...
- `ROTATE_ERROR` - Invalid input

**Notes:**
- Starts from the current position (no HOME requirement).
- `ControlRotate_RotateMoveToDegree()` is a thin wrapper around this API.
...
**Behavior:**
1. Requires HOME active at start
2. Rotates away until sensor clears
3. Continues rotating until HOME sensor triggers again
4. Completes exactly one full revolution
...
**Returns:**
- `ROTATE_RUNNING` - Motion started
- `ROTATE_ERROR` - HOME not active or invalid state
...
**Note:**
- Current implementation returns estimated degrees.
- Time-based estimate uses rotation rpm (default 1 rpm).
...
Reads the elapsed motion time in milliseconds for the active rotation.

**Behavior:**
- Returns `0` when no motion is active.
- Uses the same time base as the rotation estimator.

**Degree to Tick Relation (1 rpm example):**
- 360 degrees = 60000 ms
- 90 degrees = 15000 ms
- 10 degrees = 1667 ms (approx)

**Parameters:**
- `tick_ms_out` - Output pointer for elapsed time in ms
...
4. Session config is valid (tilt 0-75°, rotate_num >= 0 degrees)
...
- **control_rotate.c** provides time-based rotation from the current position and HOME-based RotateOne control
