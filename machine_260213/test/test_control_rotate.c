...
static void PrintStatus(const char *label)
{
    ...
    ControlRotate_ReadPosition(&pos_deg);
        ControlRotate_ReadPositionTick(&pos_tick_ms);

        printf("[%02d:%02d:%02d] %-20s  state=%d  home=%d  home_raw=%d  pos=%d deg  tick=%d ms\n",
           tm_now->tm_hour,
           tm_now->tm_min,
           tm_now->tm_sec,
           label ? label : "status",
...
/**
 * @brief Wait until HOME sensor is active.
 *
 * @param label Status label for logs.
 */
static void WaitForHomeActive(const char *label)
{
    printf("Touch metal to activate HOME (home_raw=0). ");
    printf("Release after rotation starts.\n");
